...
* `iD.OsmAbstractEntity` - common base class for `iD.osmNode`, `iD.osmWay`, `iD.osmRelation`, and `iD.osmChangeset`.

These four classes inherit from a common base, `iD.OsmAbstractEntity`.
...
The four types of entities have separate `id` spaces: a node can have the
