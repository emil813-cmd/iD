...
import { OsmAbstractEntity, type FeatureType, type OsmEntityProps } from './abstract-entity';
import { debug } from '..';
import { osmIdManager } from './id_manager';
import type { coreGraph } from '../core/graph';
import type { Vec2 } from '../geo/vector';

export type RelationMember = { type: FeatureType; id: string; role: string };

export class osmRelation extends OsmAbstractEntity {
    declare readonly type: 'relation';
    declare readonly members: RelationMember[];

    constructor(...args: Partial<OsmEntityProps & { members?: osmRelation['members'] }>[]) {
    ...
    copy(resolver: coreGraph, copies: { [id: string]: any }) {
    ...
    extent(resolver: coreGraph, memo?: { [id: string]: boolean }): geoExtent {
    ...
    // is returned, extended with an 'index' property whose value is the member index.
    memberByIdAndRole(id: string, role: string) {
        for (var i = 0; i < this.members.length; i++) {
    ...
    updateMember(member: RelationMember, index: number) {
        var members = this.members.slice();
    ...
    replaceMember(
        needle: RelationMember,
        replacement: RelationMember,
    ...
    asGeoJSON(resolver: coreGraph): GeoJSON {
            ...
            if (this.isMultipolygon()) {
                ...
                return {
                    ...
                    features: this.members.map(function (member) {
                        return Object.assign({role: member.role}, resolver.entity(member.id).asGeoJSON(resolver)) as Feature;
                    })
                };
            }
        });
