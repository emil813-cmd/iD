...
export class osmWay extends OsmAbstractEntity {
  ...
  declare readonly nodes: string[];

    constructor(...args: Partial<OsmEntityProps & { nodes?: osmWay['nodes'] }>[]) {
    ...
    areAdjacent(n1: string, n2: string) {
        for (var i = 0; i < this.nodes.length; i++) {
            if (this.nodes[i] === n1) {
    ...
    addNode(id: string, index: number) {
        var nodes = this.nodes.slice();
    ...
    updateNode(id: string, index: number) {
        var nodes = this.nodes.slice();
        var isClosed = this.isClosed();
    ...
    replaceNode(needleID: string, replacementID: string) {
        var nodes = this.nodes.slice();
        var isClosed = this.isClosed();
