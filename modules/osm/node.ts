...
export class osmNode extends OsmAbstractEntity {
    ...
    constructor(...args: Partial<OsmEntityProps & { loc?: osmNode['loc'] }>[]) {
    ...
    directions(resolver: coreGraph, projection: Projection) {
        ...
        if (this.isHighwayIntersection(resolver) && (this.tags.stop || '').toLowerCase() === 'all') {
            ...
            if (sideTag && SIDES.has(sideTag.toLowerCase()) && !hideSideTag) {
                rawValues.push({
                    type: 'side',
        ...
        const neighborNodeReducer = (neighbor: osmNode, { lookBackward = true, lookForward = true } = {}) => function(collection: { [nodeId: string]: boolean }, { nodes }: { nodes: string[] }) {
            nodes.forEach((_, i) => {
