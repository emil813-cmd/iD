...
import { debug, createEntity, osmIdManager } from '../index';
...
export type FeatureType = 'changeset' | 'node' | 'way' | 'relation';
export type FeatureTypeShort = 'c' | 'n' | 'w' | 'r';
...
export abstract class OsmAbstractEntity implements OsmEntityProps {
    ...
    abstract copy(resolver: coreGraph, copies: { [id: string]: unknown }): this;

    abstract geometry(graph: coreGraph): unknown;

    abstract extent(
        resolver: coreGraph,
        memo?: { [id: string]: boolean },
    ): geoExtent;
    ...
    abstract asJXON(changesetId: string): unknown;
